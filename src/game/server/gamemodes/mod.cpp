...
void CGameControllerMOD::OnClientDrop(int ClientID, int Type)
{
	if(Type == CLIENTDROPTYPE_BAN)
		return;
	if(Type == CLIENTDROPTYPE_KICK)
		return;
	if(Type == CLIENTDROPTYPE_SHUTDOWN)
		return;
...
{
	if(Index == TILE_ENTITY_POWERUP_NINJA)
		return false;
...
int CGameControllerMOD::ChooseInfectedClass(const CPlayer *pPlayer) const
{
	...
	while(Iter.Next())
	{
		...
		if(Iter.Player()->GetClass() == PLAYERCLASS_WITCH)
			thereIsAWitch = true;
		if(Iter.Player()->GetClass() == PLAYERCLASS_UNDEAD)
			thereIsAnUndead = true;
		if(Iter.Player()->GetClass() == PLAYERCLASS_FREEZER)
			thereIsAFreezer = true;
	}
	...
	Probability[PLAYERCLASS_WITCH - START_INFECTEDCLASS - 1] =
		(Server()->GetClassAvailability(PLAYERCLASS_WITCH) && nbInfected > 2 && !thereIsAWitch) ?
			(double) g_Config.m_InfProbaWitch :
			0.0f;
	Probability[PLAYERCLASS_UNDEAD - START_INFECTEDCLASS - 1] =
		(Server()->GetClassAvailability(PLAYERCLASS_UNDEAD) && nbInfected > 2 && !thereIsAnUndead) ?
			(double) g_Config.m_InfProbaUndead :
			0.0f;
	Probability[PLAYERCLASS_FREEZER - START_INFECTEDCLASS - 1] =
		(Server()->GetClassAvailability(PLAYERCLASS_FREEZER) && nbInfected > 5 && !thereIsAFreezer) ?
			(double) g_Config.m_InfProbaFreezer :
			0.0f;
